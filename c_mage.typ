/* This file for all magic-y type classes and abilities. */

== Mage
// Wizard, Mage, Sorcerer, Witch 

Magic is incredible and everyone else wishes they got the letter to go and study it. 
You're a wizard, hurray, so that is exactly what you do. 
Play this class to access a wide range of spells. 

Weapon die: 1d4
Stress limit: 3
Injury limit: 8

You have Advantage on tests: (Pick 3)
- Of knowledge. 
- Of perceptiveness. 
- etc. 
- To make creative use of a spell. 

*Heighten magic.*
When you cast a spell, mark stress to do one of: 
 - Reroll any one die included in casting the spell. 
 - Cast it as though you spent one additional action, up to three. 
 - Increase a threat, if the spell would instead introduce it. 
//Increase its range from close to far

*Mage's implement.*
You carry a wand, staff, orb, magical sword, book, or similar item through which your spells are focused. 
When you wield your implement and mark stress to cast a spell, you can _heighten magic_ once without marking an additional stress. 

*Complex rituals.*
Some advancement tracks grant the ability to mark Supply to conduct a _ritual_. 
When you do, you can conduct two different rituals instead of only one. 
// Encourage à aller chercher au moins 2 tracks qui donnent des rituels. 

=== Advanced skills

*Eased spells.* 
When you cast a spell and mark stress to do so, you can trade one die that results in a 1 or 2 to instead clear one stress. 
Remove the die. 

*Channeling focus.*
Mark stress to gain advantage on a test of mettle. However, you can only spend actions to cast spells during that turn. 

*Self-sustaining magic.*
Once each turn, you can mark stress to sustain a magical effect without spending an action. 
// Assuming a lot of spells ask you to sustain like Pf2e.

*Improved implement*. 
It has an associated threat. Like Blight for a serpent staff. 
 - When you wield it, "spellcasting effect" choose 2 instead of 1. 
 - Trade in a result of 1 to introduce or clear a threat instead on one target of the spell. Choose from ambient threats or the associated threat. 

*Spellcasting proficiency.*
Whenever you cast a spell, roll 1d6. 
If the result is 5 or 6, choose one effect: 
 - Reroll one damage die dealt by it
 - If you must mark stress to cast the spell, do not mark that stress. 
 - If the spell introduces a threat, increase it instead (more than one). 
/* Heightened magic table? Cast a spell, and mark stress to get an effect based on the 1d6 roll. 
1: Nothing happens. 
2-5: If the spell has you roll other dice (damage, healing or something else), you can reroll one die and keep the new result. 
6: Do not mark any stress you otherwise would upon casting the spell, including to heighten magic. 
*/


== Associated tracks
/*
Tracks focus on one type of threat. 
Summon energy, so special ability to manipulate threats. 
1. Control elements, influence ambient threats
2. Associated rituals
3. Powerful capstone for their element/threat
4. Passive that is always on, associated with their element/threat
5. Attack associated with element/threat

Each track gives the opportunity to control more doodads that will "act on their own"
  1. Ice: More blocker
  2. Fire: Constant DPS,
  3. Dread: More minions

1. Elemental Conduit - Can apply their element from ambient to a foe 
2. Elemental Transformation - Can convert targeted threat [Fire, Cold, Water, Fool, Trap] (transformer de de la glace en eau, l'eau en terrain glissant (trap), de l'eau en vapeur aveuglante(fool))
3. Spellbook
*/

=== Wizard

*Spellbook.*
Grab a stack of index cards, sticky notes or a small notebook to be your _spellbook_. 
In the fiction, your spellbook is an actual book, scroll, or tablet you can wield in one hand. 
When you wield a spellbook you can: 
 - Mark Stress to cast a spell you know, not included among your special actions. 
 - _Mark_ and _clear_ things in it. 

When rules allow you to *mark* something in the spellbook, write it down. 
When you *clear*, rip the sticky note, erase or cross it out in the notebook --- get rid of it. 

=== Seer

Nudge fate. Mark stress to reroll one die of a test, whoever attempts it. 

Mark stress to open your third eye for a short time. 
Ignore ambient Dark and Fool while it is open. 
You see through obscurity, foliage, rain, fog, curtains, and so on. Opaque objects still block your sight. 

You can draw or carve eye shapes on things around you, like a stone or tree. 
Unless it is erased or marred, you can mark Stress to see through such an eye in your vicinity for a moment. 


=== Ice Witch [Cold, Trap]
Doodad: Ice that brings up one ambient Cold 
- protects 

Spell: Icicle shard (far)
Introduce Cold, and 
1. 1d4i. 
2. 1d8i. 
3. 1d12i. 
On 8+, roll a second i of the same size. 

// Icicle Lance : Mark *Stress*, deal 1d6i per *Elemental*. Consume all targeted *Elemental* to deal 1d12i per. 
// Freeze motherf-ers: Mark *Stress*, clear all *Fool* from an ally, inflict *Cold*, *Trap* on enemy 

*Rituals of rime.* 
Mark Supply and spend a short time to conduct an ice- or cold-themed magic ritual. 
 - Add or clear an ambient Cold or Water in the vicinity. 
 - Allow someone to ignore Cold for a day. 
 - Grant the heroes the ability to walk unimpeded over snow for a day. 
 - Attempt to freeze a calm river, melt huge chunks of ice away, or similar. 
Do a Test if the application is dubious, or the effect stronger than described here. 

=== Fire Warlock [Fire, Open]
Doodad: Fireball that brings on ambient Fire
- Explodes 

=== Druid [Trap, Water]
Doodad: I am grooth

=== Artificer [Fire, Fool]
Doodad: Mechanical device (see dnd 5e)

=== Necromancer [Dread, Cold]
Doodad: Undead that brings up one dread


== Ideas for spells and features

Arcane Bolt
Mark Stress, spend 2 actions. 
2d6i and mark the target's species in your spellbook (human, bear, griffin, etc). Clear it to gain Advantage on a test related to a creature of the same kind. 

Mirrored Trio
Mark Stress to gain 3 Images. Mark an Image to clear a damage die showing an odd result. 

Magnetic field. (Passive) 
Whenever a damage die is rolled on you due to a metal weapon, arrowhead or similar, you can decide to reroll it once. 

Animal friend/Familiar. 
Magically bonded to a small animal; see through its eyes, bond to another in downtime. Stays in your vicinity. 

Scholar of ancient languages. 
You can read and understand any language, however ancient and dead. 
You can mark stress to speak a few simple words in any language. 

You are also roughly understood by elementals, the undead, and other strange and otherwise non-verbal creatures. 
This is sufficient for simple things like _go away_ or _you cannot pass_, but it does not make them any more reasonable or friendly. 
You can try though. 
_Back! The ancient tongues summon you back to the underworld, creature of the babhla leithris! Loqueris latine! Milás elliniká!_

Reputed scholar. 
Roll tests to deal with scholars, priests, lorekeepers, librarians and similar folk with Advantage. 
Anything they sell, you pay half as much for. 
_You're Kevin the wizard? Of _Treatise on the winter solstice behavior of juvenile green-striped babosaurs_ fame? I'm a fan!_

Implement of flame. "Burn something down: 2"
Spend 1 to add 1 Fire to a foe when you attack. 

Implement of ice. "Freeze something over: 2"
Spend 1 to add 1 Cold to a foe when you attack. 


Elementalist. 
Channel the elements. 
Add Fire or Cold to a foe. 
Shift one ambient Fire, Cold to a foe. 
Roll 1d4 damage for each of them. 
Bonus: Also turn one into another other. 

Fire mage spells. 
Spark. Roll 1d4 damage, add a Fire threat. 
Bonfire. Manifest a roaring fire, but mark stress absent suitable fuel. 
Clear an ambient Cold or Dark, and add ambient Fire. 
Roll 1d4 damage per Fire to a close target. 
Fireball (far). Roll up to 8d6 damage, split between your target and others close to it. No more than 3d6 per target. Add 2 ambient Fire. 

/*
Dancing lights: You can produce floating motes of light. 
Clear ambient Dark. 
Funnel ambient Dark or Fool or Trap? to any foe. 

Elemental control: Control water, fire, earth. 
Funnel ambient Fire, Water, Blight, Cold to any foe. 


Open, Dread

Add two spells: 
"Magic bolt" and "Magic trick" 
Each has options, and pick 3 from those below. 

Magic bolt. (far)
1d6i per action spent. 
Choose a threat 

Signature spells (pick 3?)
Fire: Fire blast/Light or dancing lights

Fire blast (close)
1. 1d6i. 
2. 2d6i
3. 3d6i
Trade out 1 to increase Fire. 
Roll another 1d6i for each 6. 



Light (far)
Manifest a bright light, floating in midair or following you along. 
Sustain to clear an ambient Dark for a short time. 
Water: Pressure spray/Control water

Pressure Spray (far)
Introduce Water, and 
1. 1d4i. 
2. 1d8i. 
3. 1d12i. 
On 8+, a foe is pushed back: clear one i they dealt to someone close. 

Open: Force push/Message or telepathy or mage hand

Force Push (close)
1. 1d8i
2. 2d8i, introduce Open. 
3. 3d8i, increase Open. 
If any die lands on 8, a foe is pushed back from you; clear one i they dealt to you. 

Message (vicinity)
Communicate (back and forth) through your mind. You must be aware that person is there. 

Dark: Blinding bolt/Darkness

Blinding Rays (far)
1. 1d4i, introduce Dark. 
2. 1d4i, introduce Dark to two targets.  
3. 1d4i, introduce Dark to three targets. 

Cold: Ice lance/Freezing touch of some kind?

Blight: Acid splash/Wilt and Flourish 

Acid splash (far)
1. 1d6i. 
2. 2d6i spread on foes, introduce Blight to those hit. 
3. 4d6i spread on foes, up to 2 each, introduce Blight to those hit. 

Fool: Daze/Minor illusion

Daze (far)
Introduce Fool, and 
1. 1d4i. 
2. 1d8i. 
3. 1d12i. 
On 8+, the target has a -2 penalty on its next battle roll. 

Dread: Nightmare hex/
Trap: Propelled shards/Animate object

Nightmare Hex (far)
Introduce Dread, and 
1. 1d4i. 
2. 1d6i, and the foe ...

Cantrip (far)
Manifest a minor, harmless wonder such as a tiny illusion, a puff of smoke or a gust of wind. 
You can also animate a small object to continuously do something --- stirring a pot, brushing hair, stacking coins, spinning in place. 

Sense Magic (far)
Focus on an item, creature or location. You learn if it is magical, and the level of the magical effect or an approximation of its power.
Mark stress to learn what it does in broad terms. 

Ice Shield: 
Fire blast: 
Ring of swords: 
(Whenever you get hit, add a threat to the target and deal 1d4i.)
*/
