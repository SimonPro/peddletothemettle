
= PEDDLE TO THE METTLE

(Tentative name and introduction.)

Besides a table and some friends, you will need: 
 - *Paper and pencils*; index cards or sticky notes preferred
 - *Dice sets*; multiples of d4, d6, d8, d10, d12 (number of sides), typical of tabletop games
 - *Something pleasant to eat and drink*; small pieces of finger food such as berries, nuts or candy preferred. 
 Players who provide the game master with food and drink are to be applauded each session. As a game rule. 

= KEY IDEAS

== Heroes
In the text, a *hero* is a character played by someone at the table. 
The heroes travel together as a group and ought to give themselves a name. 
Think along the lines of the _Argonauts_, or the _Knights of the round table_. 

== Stress and Injury limits
Heroes and foes in the story can sustain stress and injury, up to a limit. 
When the injuries suffered by a hero goes past its injury limit, it is dying. 
When a hero marks a Stress and meets its Stress limit, it is unable to mark more Stress until it rests. 


= TESTS
Whenever a hero does something risky, the player makes a *test* to determine the outcome. 
A test has a *difficulty*, often set by the game master, per the following table: 

== Difficulty

#table(
  columns: 2,
  align: (center, center),
  [Difficulty], [Perceived likelihood of success],
  [---], "Easy or inconsequential, do not roll",
  [4], "Likely",
  [6], "Unlikely",
  [8], "Very difficult",
  [10], "Unthinkable",
  [12], "Miraculous",
  table.hline(stroke: black)
)

The number should reflect the difficulty for a typical adult person, not that hero in particular. 

== Rolling the test
The player rolls two twelve-sided dice (2d12) and compares both results to the difficulty. 

If they are lower than the difficulty, the hero *fumbles (\--)*. 
They do not get what they want, and the situation becomes worse. 

If at least one meets or exceeds the difficulty, the hero *manages (+-)*. 
They mostly get what they want, but it may come at a cost, or come with conditions. 

If they are both equal to or higher than the difficulty, the hero *succeeds (++)*. 
They get what they want, without condition. 

In cases where the difficulty is already set but a hero has an obvious edge or hindrance, 
a roll can benefit from *Advantage* or *Disadvantage*. 

Roll an additional twelve-sided dice beyond the first two, and discard the highest (Disadvantage) or lowest (Advantage) roll(s) until two dice remain. 
More dice can be rolled if there are multiple reasons for (Dis)Advantage, and they cancel each other out when they both apply. 

/* What will scale? What should improve per level? Not damage, not roll results, ... */

= ADVENTURES

Do not precisely measure time or distances. 
When rules are concerned, there are only three distances: 
 - *Close.* "A few steps away", "in the room", "across the street". 
 - *Far.* "Down the street", "on the next floor up", "down the hallway in the next room". 
 - *In the Vicinity.* The wider area the fiction takes place in, often within earshot. "The castle", "the marketplace", "the cavern". 

As for timekeeping, the only relevant rule is a *short time*. 
A short time is long enough to last through a typical skirmish or conversation. 
However, more than a short time passes by the time the story moves to the next relevant scene. 

== Spells

*Unblocked path.*
It must be possible to trace a reasonably straight path between caster and target that is not blocked by a solid physical barrier.

*In the vicinity.*
Unless otherwise indicated, spells cease to function when the caster is no longer in the same vicinity as the spell's effect or its target.


== Death
Whenever a hero is dealt injuries that crosses past its limit, the player chooses one option: 
 - *Surrender.* The hero falls unconscious, but wakes up some time later. They could have been carried away by their friends, captured by their foes, or anything else as befits the story. Work with the game master to determine a reasonable outcome based on the circumstances. 
 - *Risk it.* The hero stays up, but continues to track injuries. If they ever exceed the limit by 10 or more, the hero dies.

== Treasure and money
Anything in the game costs either "a handful of copper" or a specific amount of gold. 
Handfuls of copper are handwaved away. A meal, a new basic spear, a kettle, a song, all cost a handful of copper. 
Only use gold when the expense is relevant. 

GOLD
1 A meaningful bribe for a guard, a potent magical potion, a horse, renting a room for a month at the inn, enough to cause a crowd of commoners to dive into a pile to fight over it. 
10 A meaningful bribe for a merchant, the price on the head of a dangerous criminal, an uncommon magical item. 
100 The deed to a house, a small dragon's cache, the services of an entire company of mercenaries for a week. 
1000 The deed to a mansion or small keep, a grown dragon's cache, a rare magical item. 

== Rest & supplies

The heroes have one shared resource: *Supply*. 
This is a rough abstraction of the camping gear, food, water, bandages and everything else they lug around. 
They can mark Supply up to a limit of *10*. Improve this limit by: 
 - *5* for each horse, camel or similar that travels with them. 
 - *50* for a ship. 

Whenever the heroes feel it is appropriate, they can mark 1 Supply to: 
- Produce some reasonable item or multiples of a small item, as though it was provisioned earlier. Snowshoes? A rope ladder? A few torches for everyone? Arrows? A potion of healing? If it costs gold, pay now. 
- Rest. Injuries and stress are reset back to zero. (Maybe a roll, make it less good.)
The heroes can do so anywhere they could reasonably sit down at least a short time to eat and drink. 
At the table, the players should also stay hydrated --- drink!

Supplies are generally replenished by being purchased. 
The cost is irrelevant, but finding a seller is. 
The heroes could buy a meal at an inn and ask for some travel rations -- that could justify a few fresh supplies. 
Raiding a poor farmer's hovel might fetch one supply too, as would hunting a deer and turning it into a kettleful of meat stew. 



= THREATS
Fire, Water, Dark, Trap, Cold, Open, Dread, Blight, Fool

=== Wording
Ambient threat: Threats that are *ambient* reflect environmental conditions. 
They are in the background. 
Unless a feature references ambient threats directly, they cannot be changed or affected. 
However, they *do* apply when conditions are checked to determine if a target has threats, and how many. 

Introduce a threat: Apply a threat directly to a target, that it did not previously have. Ambient threats are ignored for this purpose. 

Add a threat: Functions like Introduce a threat, but the same threat can be added more than once, up to 3. 

Clear a threat: Remove a threat from play, unless it is ambient. It is possible to clear a threat on someone else, when reasonable in the fiction. 

Shift a threat: Clear a threat, then Add the same threat to a different target. 

Fiction first: Notice the lack of a rule on darkness and visibility in "Dark 3". They should have a torch!

=== Definitions for ambient threats. 
Fire
1 Noon, intense heat, contained bonfire
2 Uncontrolled open fires
3 Lava pit, forest fire
(Red flame)

On someone: On fire, heatstroke, dangerous heat

Water
1 Marsh, intense humidity, rain
2 Waist-deep water, swimming, downpour
3 Underwater
(Blue drop)

On someone: Wet, currents, swimming

Dark
1 Darkened corners to skulk in, starlit night, stormy weather
2 Candlelit darkness, deep fog
3 Pitch black
(Crossed eye)

On someone: Unable to see, knocked on the head. 

Trap
1 Brambles, slick stones, danger of light injury
2 Cliffside path, thin ice, danger of serious injury
3 Deadly hidden mechanical traps, cliffside in deep snow, danger of sudden death
(Steel trap)

On someone: Nearby trap, impediments due to a trap

Cold
1 Night, near or at freezing
2 Deep snow, below freezing temperatures
3 Blizzard, arctic cold
(White snowflake)

On someone: Cold. 

Open
1 Deserted street, exposed path, little to no vegetation or clutter
2 Open area with very few features
3 Large, flat open field or plaza
(Crossed out shield)

On someone: Off guard, bent armor, isolated. 

Dread
1 Silent cave, throne room, cemetary, thunderstorm
2 Haunted or sacred places, battlefield
3 Necropolis, blood rain, plainly horrific scenes
(Black skull)

On someone: Panicked, afraid, humiliated. 

Blight
1 Poisonous mushrooms, thin air, in a tannery
2 Noxious fumes, puddles of acid
3 Toxins that quickly lead to death
(Green mushroom)

On someone: Poisoned, sick. 

Fool
1 Surprise, deception, simple illusions, confusing environments such as a busy marketplace
2 Enchantments, complex illusions, cluttered warehouse or extremely narrow alleyways
3 Dreams, strong hallucinogens, incredibly complex environments
(Question mark)

On someone: Foes hidden from you, wrong assumptions about what is real

== Skills?

Trade: Alchemy, Carpentry, Cooking, Tailoring, First aid, Haggling, and similar. 
Locale: Swim, Climb, 

= CLASSES

== Warrior
/* Fighter, Barbarian, Paladin */
Hoplites. Vikings. Legionaries. Immortals. Samurai. Eagle warriors. 
Thousands of years of real life history to pull inspiration from. 
Play this class to dish it out and take it. 

You have Advantage on tests: (Pick 3)
- Of strength. 
- Of stamina. 
- Of speed and mobility. 
- To spot and assess foes. 
- To make creative use of any weapon. 

*Sturdy.* Increase your injury limit by 5. 

*Favored weapon.* One weapon in your possession is your favored weapon. You like it, maybe you named it. 
When you attack with it, also roll a six-sided die, preferably one with a unique color or style --- this is your _favored weapon die_. 

If it lands on 6, choose 1: 
- Introduce a relevant threat (Blurry vision for Dark, knocked over for Open, panicked for Dread, and so on)
- Turn one damage die to its highest possible value (6 for a d6, 8 for a d8, and so on). 

In downtime, practice with a different weapon to replace it as your favored one. 


== Wizard
/* Wizard, Mage, Sorcerer, Witch */
Magic is incredible and everyone else wishes they got the letter to go and study it. 
You're a wizard, hurray, so that is exactly what you do. 
Play this class to access a wide range of spells. 

You have Advantage on tests: (Pick 3)
- Of knowledge. 
- Of perceptiveness. 
- 
- To make creative use of a spell. 

*Spellbook.*
Grab a stack of index cards, sticky notes or a small notebook to be your _spellbook_. 
In the fiction, your spellbook is an actual book, scroll, or tablet you can wield in one hand. 
When you wield a spellbook you can: 
 - Mark Stress to cast a spell you know, not included among your special actions. 
 - _Mark_ and _clear_ things in it. 

When rules allow you to *mark* something in the spellbook, write it down. 
When you *clear*, rip the sticky note, erase or cross it out in the notebook --- get rid of it. 

Arcane Bolt
Mark Stress, roll 1d6i per action spent. 
Mark this foe's name in your spellbook. Clear it to gain Advantage on a test related to a creature with this name. 



== Knave
/* Minor story benefit, good effect for 3-4/15 (~20-25%) rolls. */
Everyone loves a charming rogue, a brooding assassin, a morally grey mercenary with gallows humor. 
Play this class to get up to all sorts of shenanigans. 

*Suit up.*
Choose a suit: 
 - *Hearts*. Gain Advantage on tests to _charm into revealing secrets_. 
 - *Spades*. A dubious item, a worrying blood smear, a corpse --- there is always a convenient spot to bury evidence. 
 When you take a short time to "bury" something, only an extremely thorough search reveals anything. 
 - *Diamonds*. Whenever the heroes gain a sum of gold, gain 1 more or 20% more, whichever is higher. 
 - *Clubs.* Gain advantage on tests to _threaten with violence and guard doors_. 

*Lucky.*
Get a regular set of playing cards, or repurpose some index cards. 
Put together a small deck: Cards of your suit (Ace, 2 to 10, Knave, Queen, King) and two jokers, for a total of 15 cards. 
Match the Queen and King to two other heroes in your group. 
Remove one or both from the deck if there are fewer than two other heroes. 

Each time the play session begins, shuffle the deck then leave it facedown in front of you. 
*Whenever you roll a test, draw a card.* Reshuffle once they are all drawn. 

Ace: Turn a twelve-sided die of your choice to a 1. (It could be a 1 already.)
2-10: No effect. 
Knave: That's you. You automatically succeed, or put the knave back at the bottom of the deck if you already would succeed. 
Queen or King: Given you can describe a reasonable way in which the matched hero helps you succeed, gain Advantage. 
Joker: Turn a twelve-sided die of your choice to a 12. 


== Disciple
/* Cleric, Druid, Bard, Warlock */
You have faith in a deity, follow the teachings of a powerful patron, or see yourself as the herald of natural forces. 
You are a conduit for this outside power, or a student of it. 
Play this class to double down on a thematic character concept and tie it to the fantasy from the start. 

*Domains.* Choose two of the following elements that best reflect the nature of what you follow or worship. 
They are your two _domains_. 
For instance, a disciple of a goddess of harvests could choose Rest and Wild. 
A student of a bardic school could choose Bold and Lore. 

Bold - Awe, Passion. 
Arms - War, Authority. 
Magic - Magic, Strange. 
Fire - Zeal, Destruction. 
Hoax - Night, Dreams. 
Lore - Knowledge, Secrets. 
Rest - Healing, Death: Heal, deal damage to undead. 
Tinker - Craftsmanship, Civilization. 
Wild - Nature, Sea: Heal and damage mixed. 

*Mantras.*
Write 2 mantras that players can repeat or respond to, one per domain. Teach them to the other players. 

Tough. _When the going gets tough_: (targeted player) _The tough get going._
Magic. _Praise be to Jess_: (targeted player) _Matron of Magic and Tasteful Cupcakes._
Fire. _Fireball!_: (targeted player) _Taaa, ta ta, da ta da_
Arms. _Swords out for our Lord Aramde_: (other players thump their chests)

Any player-hero who responds gains Advantage on that test. 
You can say each mantra once, and must rest before you say it again. 

*Icon of Devotion.* 
Object you wield - a religious icon, a musical instrument, a sprig of mistletoe. It's not a weapon at all. 
When you wield it, reroll 1 and 2 on tests relevant to your domains. 


== Druid
This class may contain nuts. 
Seriously, if there are nut allergies at your table, substitute them with candy or play something else. 

Procure a bag of peanuts, cashews or other crunchy delight. 
If you can easily procure them each session, berries are also acceptable. 

You dispense blessings: metaphorical (advice) or physical (magical berries). 
Your nuts also count as threat tokens?

COMMANDER
Battle tactic: Swap two damage dice between allies that are close together OR swap two damage dice between foes OR grant an ally one additional action. 
Fall back!: Ally spends one action to clear one damage die. 
To me!: Allies that are far can get close to you. 

Reroll 1s on tests of Mettle. 


ROGUE
(+1d12) Dodge. 
Advantage to Mettle under ambient Dark or Fool. 
Evade. Mark Stress to clear one damage die. 

Sneak attack. If you next action this turn deals any damage, also deal 1d12d if the target has Dark or Fool. 

ILLUSIONIST
Mirrored Trio: Mark Stress to gain 3 Images. Mark an Image to clear a damage die showing an odd result. 

ABJUROR
Magnetic field. (Passive) Whenever a damage die is rolled on you due to a metal weapon, arrowhead or similar, you can decide to reroll it once. 


== BATTLES

Sequence: 
1. Test of Mettle. 
2. Enemies act out their "suspended actions"
3. Hero with the highest Mettle result takes its turn; then identifies the next player to take its turn. And so on. 
Note: "Reactions" do not exist in the sense of being a distinct activity. Just spend all your actions. 
4. Settle. 

=== Test of Mettle
Special test: 
Fumble; 1 action
Manage; 2 actions (Mark Stress to turn into Success is possible)
Succeed; 3 actions
Difficulty is the foe with the highest difficulty. 

Can have a unique description, but is just "Be ready to fight" by default. 

==== Surprise
In the first round, each hero or foe gets one action, then Settle. 

=== Actions

Move: Get somewhere far. 

Interact: With your equipment or the environment. 

Defend: Reroll a damage die on you, keeping the new result if lower OR Shift a damage die from a close ally to you. 

Attack. Deal 1d4i close to you, with your fists or an improvised weapon. 
Weapons grant an Attack ability that overrides this one. 
/* No limit to multiple attacks, just have to make introducing or clearing threats a priority. */

Introduce a threat. The best suited one based on your description. 

Blight - Let your poisoned weapon take effect. 
Cold - Rip off their fur cloak. 
Dark - Throw pocket sand, blinding them. Position yourself in shadows. 
Dread - Scare or humiliate them. 
Fire - Smash them with a burning torch. 
Fool - Make a feint. 
Open - Place yourself in cover from them, shove them prone, reposition to flank them. 
Trap - Lure them to a dangerous spot. 
Water - Splash water at them.  

Clear a threat. Based on your description, which must be justified in the fiction and could be subject to a test when far fetched. 

Cast a spell. Some spells require more than one action to cast, though it is one by default. 

/*
Interesting but complex... maybe for a class?
Focus
Some actions indicate you _lose focus_. The focus action lets you regain it. 
You cannot do something that causes you to lose focus if you have lost it. 
*/

== EQUIPMENT
You have 2 wielded items, Interact to switch to a different set. 

=== 1-handed items (Wield any two.)

Shield
Raise shield. Reroll any number of damage dice on you OR clear one. 

Melee weapon
Attack. Deal 1d8i close to you. 
_Swords, spears, axes, and various other weapons wielded in one hand._

Off-hand weapon
Backhand attack. Deal 1d4i close to you. 

Implement
Blast. Deal 1d4i close or far from you. 
_Scepters, wands and similar item that can channel magical energy into a forceful blast or spells._

Wand of icy gusts
As wand, and mark stress to add Cold (up to 1). 

Standard
Rally. Mark Stress to grant all heroes advantage on a test of Mettle. Take this action when it is rolled, and it counts as your first one this round. 
_A banner or other ostentatious symbol, meant to strengthen resolve and serve as an impressive display._

Magical trinket
Focus. Reroll any number of damage dice you dealt with one spell this turn. 
_Often a spellbook, scroll, prayer wheel, religious symbol or other such magical aid._

=== 2-handed items (Wield only one.)

Heavy weapon
(Adv) Break something down, or open. 
Attack. Deal 1d12i close to you. 
_Greatclubs, longswords, greataxes and various other weapons that require both hands to strike._

Bow
Shoot. Deal 1d8i close or far from you. 

/*
= GRAB BAG OF IDEAS

Rogue. 
Double daggers. 
(+1d12) Perform a knife trick. 
(Ability) Double slice. Roll 2d4 damage on a foe close to you. 
(Ability) Deadly feint. Apply 1 Fool, or mark stress to roll 1d4 damage for each Fool. 
Dirty tricks. 
When you Introduce a threat, mark Stress to also add one of Dark, Open, or Fool. 


Massive weapon.
Damage die: 1d10 close
(+1d12) Break something down. 
Mark stress to reroll your damage die. 

Archer. 
(+1d12) Perform a bow trick. 
(Ability) Shoot. Roll 1d8 damage on a foe far from you, or 1d4 when close. 
Mark stress to reroll your damage die. 
Don't track arrows, unless they have a special effect. 

Fighter. 
Roll an attack, reroll any one die. 

Blindfight: Clear all Dark threats. 
Tough: +1 injury limit
Intercept; at resolve, bring any number of "1s" to you from close range. 

Ranger. 
(Action) Scouting flashback
Mark stress. Clear all Trap threats, or shift up to 2. 
_Thankfully, I had seen this while scouting ahead and instead led him into a trap of my own making._

Lure into a trap
Add Trap, or a threat found among ambient ones, to a foe. 

Death by a thousand threats
Mark stress. 
Roll 1d12. If the result on that die does not equal or exceed the number of threats on the foe, repeat. 
Continue to add dice one at a time until a result equals or exceeds threats. 


Lorebook. 
(+1d12) Recall lore. 
(Ability) Attack. Roll 1d4 damage on a foe close to you. 
Mark stress to reroll one damage die included in a spell. 

Implement of flame. "Burn something down: 2"
Spend 1 to add 1 Fire to a foe when you attack. 

Implement of ice. "Freeze something over: 2"
Spend 1 to add 1 Cold to a foe when you attack. 

Double daggers. "Skilled knife trick: 2"
Spend 1 to roll a second damage die. 

Elementalist. 
Channel the elements. 
Add Fire or Cold to a foe. 
Shift one ambient Fire, Cold to a foe. 
Roll 1d4 damage for each of them. 
Bonus: Also turn one into another other. 

Fire mage spells. 
Spark. Roll 1d4 damage, add a Fire threat. 
Bonfire. Manifest a roaring fire, but mark stress absent suitable fuel. 
Clear an ambient Cold or Dark, and add ambient Fire. 
Roll 1d4 damage per Fire to a close target. 
Fireball (far). Roll up to 8d6 damage, split between your target and others close to it. No more than 3d6 per target. Add 2 ambient Fire. 

Armor reduces damage you take at Settle, 1 for 1. 

== Damage dice
Beware the central limit theorem. 

1d4 Small edge case. 
1d6 Small weapon. 
1d8 Normal weapon. 
2d4 Dual daggers. 
1d10 Heavy weapon. 
2d6 Some ability. 
1d12 Super heavy weapon. 
2d8 Cool ability. 
2d10 Resource burn ability. 
2d12 Major thing, huge spell. 
(10 steps, but can be spread around ie 3d8 against as many creatures.)

== Resolution mechanic
Ideally, no static modifiers, just adding dice pools like damage. 
Like a total difficulty, you "win" when everyone has rolled it together. 
Everything is a group challenge, in a way? Unless attempted solo. 

= CREATURES
Swarm: Any damage die higher than 2 counts as 2. 
Giant: Any damage die lower than 4 is ignored, if dealt from below the knee. 
Tough hide: Any damage die of 2 or lower is ignored. 


==== Battle below the open sea of the Arctic, at night (extreme!)
Cold 3, Water 3, Open 2, Dark 2
*/


= BESTIARY
/* See Design notes for stat block design. */


*Venomous dragon*
Difficulty 8; hide from 10. 
Flies away to its lair at 30i, dies at 50i. 
Stress limit: 4
Passive. Ignores Blight. 
Gets an additional action for each foe that fumbles its Mettle test. 

Mettle
Stand still, buffeted by its wings. Hold your breath through toxic fumes. 

Act
Inhale. Allows one use of Exhale. 
Blighted breath. Exhale, deal 2d12i and add Blight to one target per action spent. 
Sickening roar. Exhale, heroes with Blight lose one action. 
Venomous claws. 2d8 split between close foes, add Blight on 6 or more. 
Twisting tail. Any time a foe moves to or from somewhere close this round, deal 1d6i. 
Move. Runs, swims and flies equally well. 

Settle
Tough hide. Clear all damage dice of 3 or less. 
Caustic venom. All foes injured suffer Blight more injuries. 

/*
Gore. 1d8i, add Open. The gored foe is stuck on the horn until it clears Open. (One gored foe at a time.)
*/

